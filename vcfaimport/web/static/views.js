      ...
      <div class="grid two mt">
        <div class="card"><div class="card-h"><h3>Waves</h3><span class="sub">run in order, lowest first</span>
          ...
          ${d.waves.length ? html`<div class="table-wrap"><table class="t"><thead><tr><th>Wave</th><th style="width:40%">Progress</th><th class="right">VMs</th><th class="right">Done</th><th class="right">Failed</th><th></th></tr></thead><tbody>
            ${d.waves.map((w) => html`<tr><td class="nowrap"><b>Wave ${w.wave}</b></td><td>${segbar(w.counts, w.total)}</td><td class="right num">${n(w.total)}</td>
...
VIEWS.discover = view({
  ...
  init(v) {
    ...
    v.form = v.form || { server: vc.server || '', user: vc.user || '', password: '', insecure: false, powered_on: false, no_tools: false, concurrency: 12 };
  },
  async load(v) {
    const [o, d] = await Promise.all([GET('/api/overview'), GET('/api/discovered')]);
    v.o = o;
    v.vms = d.vms;
  },
  paint(v) {
...
VIEWS.select = view({
  ...
  init(v, p) {
    v.f = v.f || { q: '', dc: null, folder: null, cluster: '', network: '', power: '', tools: '', show: 'all' };
  ...
  filtered(v) {
    ...
    return v.vms.filter((x) => {
      ...
      if (q.length) {
        const hay = [x.name, x.moref, x.cluster, x.folder, x.networks, x.guest_os, x.namespace].join(' ').toLowerCase();
        ...
        <div class="card">
          ...
          <div class="table-wrap"><table class="t">
            <thead><tr><th class="chk"><input type="checkbox" data-act="selPage" title="Select this page" ${attr(allOnPage, 'checked')}></th>
              ${th(v, 'name', 'VM')}${th(v, 'power_state', 'Power')}${th(v, 'folder', 'Folder')}${th(v, 'cluster', 'Cluster')}${th(v, 'networks', 'Networks')}
              ${th(v, 'cpu_count', 'Size')}${th(v, 'tools_status', 'Tools')}${th(v, 'namespace', 'Namespace')}${th(v, 'wave', 'Wave')}<th>Queue</th></tr></thead>
            <tbody>${pageRows.map((x, i) => html`<tr class="${x.selected ? 'on' : ''}">
              ...
              <td><span class="tag ${x.power_state === 'POWERED_ON' ? 'ok' : ''}">${(x.power_state || '?').replace('POWERED_', '').toLowerCase()}</span></td>
              <td class="small clip" title="${x.folder}">${x.folder || html`<span class="faint">/</span>`}</td><td class="small nowrap">${x.cluster}</td>
              ...
              <td class="small ${x.tools_status && !/running/i.test(x.tools_status) ? 'warn-text' : 'muted'}">${(x.tools_status || '—').replace(/^TOOLS_/, '').toLowerCase()}</td>
...
VIEWS.stage = view({
  ...
  async load(v) {
    ...
    v.saved = JSON.stringify([v.fr, v.nr]);
  ...
  paint(v) {
    ...
    const dirty = JSON.stringify([v.fr, v.nr]) !== v.saved;
    const nsOptions = uniq([].concat(v.fr.map((r) => r.namespace), v.nr.map((r) => r.namespace), p.records.map((r) => r.namespace))).sort();
    return html`
      <div class="note info">A VM's <b>namespace</b> comes from, in order: the namespace set on it in Select, the folder map (most specific folder wins), the portgroup map, then the default below.
  ...
  act: {
    cell(t, e, v) {
      const rows = t.dataset.m === 'f' ? v.fr : v.nr;
      rows[+t.dataset.i][t.dataset.col] = t.value;
      schedulePreview(v);
    },
    addRow(t, e, v) {
      const m = t.dataset.m;
      const rows = m === 'f' ? v.fr : v.nr;
      ...
      rows.push(row);
      v.render();
      const focus = $('#' + m + '-' + (rows.length - 1) + '-' + (t.dataset.key ? (m === 'f' ? 'namespace' : 'subnet') : (m === 'f' ? 'folder' : 'portgroup')));
      if (focus) focus.focus();
      schedulePreview(v);
    },
    delRow(t, e, v) { (t.dataset.m === 'f' ? v.fr : v.nr).splice(+t.dataset.i, 1); v.render(); schedulePreview(v); },
    ...
    async saveMaps(t, e, v) {
      const m = await PUT('/api/maps', { folder_rows: v.fr, network_rows: v.nr });
      v.saved = JSON.stringify([v.fr, v.nr]);
      v.maps = m;
      toast('Maps saved', m.folder.path + ' · ' + m.network.path, 'ok');
      v.render();
    },
    async stage(t, e, v) {
      await runPreview(v);
...
function stageBody(v) {
  return { folder_rows: v.fr, network_rows: v.nr, default_namespace: v.defaults.ns, default_wave: parseInt(v.defaults.wave, 10) || 1 };
}
...
function mapTable(v, m) {
  ...
  if (!rows.length) return html`<div class="card-b small muted">No entries yet. Use <b>+ Map</b> next to a folder or network above, or add one.</div>`;
  return html`<div class="table-wrap" style="max-height:340px" data-scroll="map-${m}"><table class="t maptable"><thead><tr>${cols.map((c) => html`<th>${c[1]}</th>`)}<th></th></tr></thead><tbody>
    ${rows.map((r, i) => html`<tr>${cols.map(([k, , list]) => html`<td style="${k === 'wave' || k === 'device_key' ? 'width:70px' : ''}"><input class="input ${k === 'folder' || k === 'portgroup' ? 'mono' : ''}"
...
VIEWS.waves = view({
  ...
  init(v) { v.sel = new Set(); v.open = v.open || new Set(); v.q = ''; },
...
async function moveTo(v, morefs, wave) {
  const moving = morefs.filter((m) => { const x = v.byMoref.get(m); return x && x.wave !== wave; });
  if (!moving.length) return;
  try {
    const r = await POST('/api/vms/wave', { morefs: moving, wave });
    toast('Moved ' + plural(r.moved, 'VM') + ' to wave ' + wave, r.refused.length ? r.refused.length + ' locked: ' + r.refused.slice(0, 2).join('; ') : '', r.refused.length ? 'warn' : 'ok');
    v.sel.clear();
  } catch (err) { fail(err); }
  await v.refresh();
}
function waveColumn(v, w, idx, waves, hitSet, q) {
  ...
  return html`<div class="wcol" data-dropwave="${w}">
    <div class="wcol-h">
      ...
      <div class="row wrap tiny">${nss.slice(0, 3).map((x) => html`<span class="tag mono">${x}</span>`)}${nss.length > 3 ? html`<span class="tag">+${nss.length - 3}</span>` : ''}</div>
    ...
    <div class="wcol-b" data-scroll="wave-${w}">${Object.keys(groups).sort().map((folder) => {
      ...
      return html`<div class="fgroup">
        ...
        ${open ? html`<div class="fgroup-b">${g.map((x) => html`<div class="vmrow ${x.locked ? 'locked' : ''} ${hitSet.has(x.moref) ? 'hit' : ''}" ${x.locked ? '' : raw(`draggable="true" data-drag="vm" data-moref="${esc(x.moref)}"`)}>
          ...
          ${dot(x.state)}<span class="name" title="${x.vm_name} · ${x.moref} · ${label(x.state)}">${x.vm_name}</span>
...
VIEWS.execute = view({
  ...
  act: {
    ...
    async start(t, e, v) {
      ...
      const ok = await modal({
        title: (opt.dry_run ? 'Dry run: ' : '') + (imp ? 'Import ' : 'Precheck ') + plural(pl.total, 'VM') + '?',
        danger: risky, ic: imp ? 'execute' : 'shield', confirm: opt.dry_run ? 'Start dry run' : (imp ? 'Import' : 'Start precheck'),
        typeWord: risky ? 'IMPORT' : null, wide: true,
        body: html`${planTable(v)}
...
function planTable(v) {
  ...
  return html`<table class="t compact" style="border:1px solid var(--line);border-radius:8px"><thead><tr><th>Wave</th><th class="right">VMs</th><th class="right">Batches</th><th>Namespaces</th></tr></thead><tbody>
    ${pl.plan.map((p) => html`<tr><td><b>${p.wave}</b></td><td class="right num">${n(p.vms)}${p.vms !== p.eligible ? html` <span class="faint">of ${n(p.eligible)}</span>` : ''}</td><td class="right num">${n(p.batches)}</td>
      <td class="small mono">${p.namespaces.join(', ')}</td></tr>`)}
    <tr><td><b>Total</b></td><td class="right num"><b>${n(pl.total)}</b></td><td class="right num"><b>${n(pl.plan.reduce((a, p) => a + p.batches, 0))}</b></td><td></td></tr></tbody></table>`;
}
function goButton(v) {
  const pl = v.plan, imp = v.opt.stage === 'import', busy = !!(S.pulse && S.pulse.job);
  const label2 = (v.opt.dry_run ? 'Dry run ' : '') + (imp ? 'Import ' : 'Precheck ') + plural(pl ? pl.total : 0, 'VM');
  return html`<button class="btn ${imp && !v.opt.dry_run ? 'danger' : 'primary'}" data-act="start" ${attr(busy || !pl || !pl.total, 'disabled')}>${icon('execute')} ${label2}</button>
    <span class="small muted">${busy ? 'Another job is running.' : 'You will review and confirm first.'}</span>`;
}
...
VIEWS.queue = view({
  ...
  init(v, p) {
    v.f = { states: new Set(p.state ? p.state.split(',') : []), q: p.q || '', wave: p.wave || '', ns: p.ns || '', folder: '' };
  ...
  paint(v) {
    ...
    const base = v.vms.filter((x) => (!f.wave || String(x.wave) === f.wave) && (!f.ns || x.namespace === f.ns) && (!f.folder || inFolder(x.folder || '', f.folder)) &&
      (!q || [x.vm_name, x.moref, x.message, x.batch_name, x.precheck_batch, x.folder].join(' ').toLowerCase().includes(q)));
    ...
    return html`<div class="card">
      ...
      <div class="table-wrap"><table class="t"><thead><tr>
        ...
        ${th(v, 'vm_name', 'VM')}${th(v, 'state', 'State')}${th(v, 'wave', 'Wave')}${th(v, 'namespace', 'Namespace')}${th(v, 'folder', 'Folder')}${th(v, 'batch_name', 'Batch')}${th(v, 'attempts', 'Att.')}${th(v, 'message', 'Message')}${th(v, 'updated_at', 'Updated')}</tr></thead>
        <tbody>${pageRows.map((x) => html`<tr class="click ${v.checked.has(x.moref) ? 'on' : ''}" data-act="vm" data-moref="${x.moref}">
          ...
          <td><span class="vm-name">${x.vm_name}<small>${x.moref}</small></span></td><td>${pill(x.state)}</td><td class="num">${x.wave}</td>
...
function jobsTab(v) {
  ...
  return html`<div class="grid side">
    <div class="card"><div class="joblist" data-scroll="jobs">${v.jobs.map((x) => html`<div class="jobitem ${x.id === v.jobId ? 'on' : ''}" data-act="pickJob" data-id="${x.id}">
      ${statusIcon(x.status)}<div class="grow" style="min-width:0"><div class="t ellipsis">${x.title}</div><div class="m">${fmtTime(x.started_at, true)} · ${fmtDur(elapsed(x))}</div></div></div>`)}</div></div>
    <div class="card">${j ? html`<div class="card-h">${statusIcon(j.status)}<div class="grow"><h3>${j.title}</h3>
        <div class="sub">${STATUS_WORD[j.status] || j.status} · started ${fmtTime(j.started_at, true)} · ${fmtDur(elapsed(j))}${j.error ? html` · <span class="bad-text">${j.error}</span>` : ''}</div></div>
...
function eventsTab(v) {
  ...
  const rows = v.events.filter((e) => !q || [e.message, e.moref, e.batch].join(' ').toLowerCase().includes(q));
  return html`<div class="card"><div class="toolbar">
    ...
    <div class="table-wrap"><table class="t compact"><thead><tr><th>When</th><th>Level</th><th>VM</th><th>Batch</th><th>Message</th></tr></thead><tbody>
    ${rows.slice(0, 500).map((e) => html`<tr><td class="nowrap small muted">${fmtTime(e.ts, true)}</td><td><span class="tag ${e.level === 'error' ? 'bad' : e.level === 'warn' ? 'warn' : ''}">${e.level}</span></td>
